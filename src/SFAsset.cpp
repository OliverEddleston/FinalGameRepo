...
SFAsset::SFAsset(SFASSETTYPE type) {
  ...
  case SFASSET_BARRIER:
    tmp_surf = IMG_Load("assets/barrier.png");
    break;
  }
...
void SFAsset::OnRender(SDL_Surface * level) {
  // 1. Get ttmphe SDL_Rect from SFBoundingBox
  SDL_Rect rect;
...
void SFAsset::GoEast() {
  Vector2 c = *(bbox->centre) + Vector2(5.0f, 0.0f);
  if(!(c.getX() > (SDL_GetVideoSurface()->w)-35)) {
    bbox->centre.reset();
    bbox->centre = make_shared<Vector2>(c);
  }
}

// Making the collectable move from west to east & east to west
bool check = false;
void SFAsset::CoinMovement() {
if(check == false){
  Vector2 c = *(bbox->centre) + Vector2(5.0f, 0.0f);
...
void SFAsset::HandleCollision() {
  if(SFASSET_PROJECTILE == type || SFASSET_ALIEN == type || SFASSET_COIN == type ||SFASSET_PLAYER || SFASSET_BARRIER) {
    SetNotAlive();
  }
}
